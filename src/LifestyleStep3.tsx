...
export type ProductTypeOption = 'capsules' | 'gummies' | 'drops' | 'powder' | 'skincare' | 'device';
...
export interface Step3Values {
  ...
  productOutputFormat: '1x1' | '4x5' | '16x9' | null;
...
const PRODUCT_SETUP_TYPE_OPTIONS: ProductTypeOption[] = ['capsules', 'gummies', 'drops', 'powder', 'skincare', 'device'];
const PRODUCT_SETUP_COUNT_OPTIONS: ProductCountOption[] = [1, 2, 3, 4];
const PRODUCT_SETUP_SCALE_OPTIONS: ProductScaleOption[] = ['small', 'realistic', 'hero'];
const PRODUCT_VIEW_OPTIONS: Array<{ label: string; value: ProductViewPreset; tooltip: string }> = [
  {
    label: 'Front View',
    value: 'front',
...
const PRODUCT_COMPOSITION_OPTIONS: Array<{
  label: string;
  value: Step3Values['productCompositionPreset'];
  tooltip: string;
}> = [
  {
...
const PRODUCT_LIGHTING_OPTIONS: Array<{
  label: string;
  value: Step3Values['productLighting'];
  tooltip: string;
}> = [
  {
...
const PRODUCT_OUTPUT_FORMAT_OPTIONS: Array<{
  label: string;
  value: Step3Values['productOutputFormat'];
  tooltip: string;
}> = [
  {
    label: '1:1 (Square)',
...
const PRODUCT_INTERACTION_EDITORIAL_OPTIONS: Array<{
  label: string;
  value: NonNullable<Step3Values['productInteractionEditorial']>;
  tooltip: string;
}> = [
  {
...
type UGC_STYLE_OPTION = 'optimized' | 'natural';
const UGC_STYLE_OPTIONS: Array<{ value: UGC_STYLE_OPTION; label: string; description: string }> = [
  {
    value: 'optimized',
    label: 'UGC Optimized',
    description: 'Balanced lifestyle tone with minor polish; still human-first and unstaged.'
  },
  {
    value: 'natural',
    label: 'Natural UGC',
    description: 'No corrections or enhancement—only commercial/studio cues are blocked.'
  }
];
...
const LifestyleStep3: React.FC<LifestyleStep3Props> = ({
  ...
  const effectiveProductMode = isProductMode || values.productMode;
  ...
  const bundlesPanel = productCount > 1 && (
  ...
  const productSetupSection = (
    <AccordionSection
      icon={Film}
      title="Product Setup"
      tooltip="Define the hero SKU, packaging, and scale without involving people"
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {PRODUCT_SETUP_TYPE_OPTIONS.map(option => (
              <button
                ...
                title={`Sets the hero product to ${option}. Does not adjust lighting, props, or composition.`}
              >
        ...
        <div className="space-y-3 rounded-lg border border-gray-700 bg-gray-800/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[11px] text-gray-500">Select how many SKUs in frame; does not imply people or grip.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {PRODUCT_SETUP_COUNT_OPTIONS.map(option => (
              <button
                ...
                title={`Places ${option} product${option > 1 ? 's' : ''} in the layout. Does not add hands or creators.`}
              >
                {option}
              </button>
            ))}
        ...
        <div className="flex items-center justify-between rounded-lg border border-gray-700 bg-gray-800/20 p-4">
          <div>
            ...
            <p className="text-[11px] text-gray-500">Toggle on for grouped kits. Does not add narrative interactions.</p>
          </div>
          <button
            type="button"
            role="switch"
            ...
            title="Indicates a bundle or routine layout without introducing hands, people, or UGC tone."
          >
            <span
          ...
          <div className="flex flex-wrap gap-2">
            {PRODUCT_SETUP_SCALE_OPTIONS.map(option => (
              <button
                ...
                title={`Focuses on a ${option} scale presentation. Does not reframe the shot or introduce hands.`}
              >
                {option.charAt(0).toUpperCase() + option.slice(1)}
              </button>
            ))}
  ...
  const isCustomViewActive = values.productViewCustomText !== null;
  const selectProductViewPreset = (value: ProductViewPreset | 'custom') => {
    if (value === 'custom') {
      updateValue('productViewPreset', null);
      updateValue('productViewCustomText', values.productViewCustomText ?? '');
    } else {
      updateValue('productViewPreset', value);
      updateValue('productViewCustomText', null);
    }
      ...
      <div className="space-y-4">
        <div className="space-y-3 rounded-lg border border-gray-700 bg-gray-800/20 p-4">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[11px] text-gray-500">Only the camera position changes; lighting, styling, and background remain untouched.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={getPillClass(isCustomViewActive)}
  ...
  const setCompositionPreset = (value: Step3Values['productCompositionPreset']) => {
    updateValue('productCompositionPreset', value);
        ...
        <div className="flex flex-wrap gap-2">
          {PRODUCT_COMPOSITION_OPTIONS.map(option => (
            <button
              key={option.value}
              type="button"
              onClick={() => setCompositionPreset(option.value)}
  ...
  const setEcommerceAlignment = (value: Step3Values['ecommerceAlignment']) => {
    updateValue('ecommerceAlignment', value);
      ...
      <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {ECOMMERCE_ALIGNMENT_OPTIONS.map(option => (
              <button
                key={option.value}
                type="button"
                onClick={() => setEcommerceAlignment(option.value)}
        ...
        <div className="flex items-center justify-between rounded-lg border border-gray-700 bg-gray-800/20 p-3">
          ...
          <button
            ...
            onClick={() => {
              toggleBooleanFlag('reserveBlankSpace');
              markSectionTouched('ecommerceLayout');
            }}
  ...
  const setEnvironment = (value: Step3Values['productEnvironment']) => {
    updateValue('productEnvironment', value);
    markSectionTouched('productEnvironment');
  };
  const setProductLighting = (value: Step3Values['productLighting']) => {
    updateValue('productLighting', value);
    markSectionTouched('lighting');
  };
  const setProductOutputFormat = (value: Step3Values['productOutputFormat']) => {
    updateValue('productOutputFormat', value);
    markSectionTouched('outputFormat');
  };
      ...
      <div className="space-y-4">
        <div className="space-y-3 rounded-lg border border-gray-700 bg-gray-800/20 p-4">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              Select backgrounds that keep the product isolated or on clean surfaces. No lifestyle rooms or people.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="space-y-2 rounded-lg border border-gray-700 bg-gray-900/30 p-3">
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              Hint the color or tonal direction of the background. Does not force a literal color unless the environment allows it.
            </p>
          </div>
          <input
            type="text"
  ...
  const setEditorialInteraction = (value: Step3Values['productInteractionEditorial']) => {
    updateValue('productInteractionEditorial', value);
  ...
  const productInteractionEditorialSection = (
    ...
    >
      <div className="space-y-3 rounded-lg border border-gray-700 bg-gray-900/60 p-4">
        <p className="text-[11px] text-gray-500">
          Hands are editorial tools, not people. Choose how the product is held, opened, or placed.
        </p>
        <div className="flex flex-wrap gap-2">
          {PRODUCT_INTERACTION_EDITORIAL_OPTIONS.map(option => (
            <button
              key={option.value}
              type="button"
              onClick={() => setEditorialInteraction(option.value)}
  ...
  const productLightingSection = (
    <AccordionSection
      icon={Sun}
      title="Lighting"
      tooltip="Control illumination clarity without referencing time or mood"
    ...
    >
      <div className="space-y-3">
        <p className="text-[11px] text-gray-500">
          Define the technical light source for product clarity. No emotional or lifestyle language—just illumination behavior.
        </p>
        <div className="flex flex-wrap gap-2">
          {PRODUCT_LIGHTING_OPTIONS.map(option => (
            <button
              ...
              type="button"
              onClick={() => setProductLighting(option.value)}
  ...
  const productOutputFormatSection = (
    <AccordionSection
      ...
      isTouched={touchedSections.has('outputFormat')}
    >
      <div className="space-y-3">
        <p className="text-[11px] text-gray-500">
          Pick the renderer framing. This only affects export crop; camera or composition stay unchanged.
        </p>
        <div className="flex flex-wrap gap-2">
          {PRODUCT_OUTPUT_FORMAT_OPTIONS.map(option => (
            <button
              key={option.value ?? 'none'}
              type="button"
              onClick={() => setProductOutputFormat(option.value)}
  ...
  const advancedSection = (
    <AccordionSection
      icon={Scissors}
      title="Advanced (Pro)"
      tooltip="Hero personas, camera composition, and bundle controls"
  ...
  const isPersonDisabled = values.noPerson || effectiveProductMode;
  ...
  useEffect(() => {
    setSceneMode(effectiveProductMode ? 'product' : 'ugc');
  }, [effectiveProductMode]);

  // Block and clear UGC state when Product Mode is active
  useEffect(() => {
    if (!effectiveProductMode) return;
  ...
  useEffect(() => {
    if (isProductMode && !values.productMode) {
      updateValue('productMode', true);
    }
  }, [isProductMode, values.productMode, updateValue]);
  ...
  useEffect(() => {
    const shouldDisablePerson =
      (isProductMode || values.productMode) || values.formulationStoryEnabled;
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-4 p-4">
      ...
      <div className="flex flex-col gap-2 pt-2 pb-4 border-b border-b-gray-700">
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-gray-700/80 bg-gray-900/60 px-4 py-3">
          <div>
            <p className="text-sm font-semibold text-white">Product Mode</p>
            <p className="text-xs text-gray-400">Switch to ecommerce-only production—no UGC people or candid actions.</p>
          </div>
          <button
            type="button"
            role="switch"
            aria-checked={effectiveProductMode}
            onClick={() => toggleBooleanFlag('productMode')}
            className={`relative h-6 w-11 rounded-full transition ${effectiveProductMode ? 'bg-indigo-500' : 'bg-gray-600'}`}
          >
            <span className={`absolute left-1 top-1 block h-4 w-4 rounded-full bg-white shadow transition ${effectiveProductMode ? 'translate-x-5' : 'translate-x-0'}`} />
          </button>
        </div>
        <p className="text-[11px] text-gray-500">
          Product Mode hides all lifestyle/UGC controls and locks the creator off-screen. Toggle to start building ecommerce-first prompts.
        </p>
      </div>
        ...
        <>
          ...
          <div className="pt-2 pb-4 px-2">
            <div className="space-y-4">
              ...
              {!values.ugcRealMode && (
                <div className="space-y-3 rounded-lg border border-gray-700 bg-gray-900/20 p-3">
                  <div>
                    <p className="text-xs uppercase tracking-wider text-indigo-200">UGC Style</p>
                    <p className="text-[11px] text-gray-400 mt-1">
                      Natural UGC keeps the scene human and believable without studio polish. Optimized UGC is the current default tone.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {UGC_STYLE_OPTIONS.map(option => (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => {
                          updateValue('ugcStyle', option.value);
                          markSectionTouched('realism');
                        }}
                        className={getPillClass(values.ugcStyle === option.value)}
