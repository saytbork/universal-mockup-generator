...
export const UGCCaptureSituationOptions: UGCCaptureSituationOption[] = [
  ...
  { id: 'finger-invading-lens', label: 'Finger invading lens', category: 'framing' },
...
const UGCCaptureSituationText: Record<UGCCaptureSituationId, string> = {
  'lying-bed-phone-above-face': `UGC capture situation: Lying in bed, phone above face. Subject reclines on soft pillows, shoulders curling toward the mattress. Camera obeys the head, hanging just above the forehead. Phone is held in one hand above the face, the device itself remains off-frame. The free hand lifts the product toward the chin while keeping the arm cropped. Temples and chin creep into the frame while the top border closes in. Crop risk is high and focus wobble feels inevitable.`,
  ...
  'sitting-casually-tilted': `UGC capture situation: Sitting casually, phone tilted. Subject lounges with knees apart, phone angled from hip level. Camera obeys the posture tilt so the frame leans toward the floor. The phone drifts, bouncing the chin into the lower border while the legs stay wide. Kneecaps and hip bones press against the edges, creating a diagonal slice. Crop risk is high and focus prefers the thighs over the face.`,
