...
export const PHOTO_MODE_PRESETS: Record<string, string> = {
    ...
    'Foam & Texture': `Editorial macro textures used as controlled design elements: foam bubbles, gel ribbons, creamy swatches, and droplets on the set only. Product stays clean and dry; no pooling; no contact distortion; label stays fully readable. The surface plane must be visible and recede in perspective; product front face visible. Camera is eye-level or slight frontal and parallel to the surface; no top-down, no overhead, no aerial, no bird’s-eye, no flatlay.`,
...
export const LIGHTING_PRESETS: Record<string, string> = {
    ...
    'Prism Spotlight Duo': `Two controlled spotlights through prismatic modifiers (split-beam look). Visible light separation on background and edges: crisp highlight gradients, subtle spectral split on specular reflections. Keep the product and label perfectly readable: no rainbow artifacts over the label text. Studio-grade contrast.`
};
