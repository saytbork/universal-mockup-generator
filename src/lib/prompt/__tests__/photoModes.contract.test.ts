...
const makeScene = (photoMode: string): CanonicalScene => ({
  ...
  lighting: photoMode.includes('Underwater') ? 'underwater refracted directional light' : 'clinical softbox',
  specialEffects: [],
...
describe('photoModes contract', () => {
  for (const mode of photoModes) {
    it(`valid contract for mode: ${mode}`, () => {
      ...
      expect(prompt).toContain('LABEL_PROTECTION');
