...
export function buildGeometry(authority: StudioAuthorityBundle, state?: StudioUIState): string {
  ...
  if (state?.advancedControls) {
    return 'GEOMETRY_LOCK: Product references are provided in normalized frames matching the output aspect ratio. Each product maintains its exact intended width-to-height ratio. Lens behavior must be defined by CAMERA_SYSTEM_OVERRIDE when provided. Maintain rigid orthographic proportions for all products shown. Maintain zero optical distortion unless telephoto compression is explicitly selected. Fill any empty canvas space with environmental context (surfaces, backgrounds, props, atmospheric lighting effects), NEVER by stretching, compressing, or warping the product geometry.';
  }
