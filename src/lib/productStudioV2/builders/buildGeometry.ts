...
export function buildGeometry(authority: StudioAuthorityBundle, state?: StudioUIState): string {
  ...
  // Orientation: buildProductOrientation is the sole emitter of PRODUCT_ORIENTATION_LOCK.
  // When rotation is user-defined we still emit a lightweight note here for geometry context,
  // but we never duplicate the full PRODUCT_ORIENTATION_LOCK guardrail.
  ...
  const orientationBlocks = rotationEnabled
    ...
    : [
        // VERTICAL_AXIS_ALIGNMENT and CAMERA_ORIENTATION_LOCK are geometry-domain constraints;
        // PRODUCT_ORIENTATION_LOCK itself is owned exclusively by buildProductOrientation.
