...
function buildSupplementsPhysicalRules(state: any): string {
  ...
  switch (def.kind) {
    case 'capsules': {
      ...
      parts.push(`CAPSULE_STYLE: ${capsuleStyle} capsules${contentColor ? ` with ${contentColor} powder/liquid contents` : ''}.`);
    ...
    case 'gummies': {
      ...
      parts.push(`GUMMY_DESCRIPTION: ${shape}-shaped gummies${gummyColor ? ` in ${gummyColor} color` : ''}.`);
...
export const supplementsProfile: IndustryProfileModule = {
  id: 'supplements',
  allowedPhotoModes: [],
  resetState: () => ({
    props: '',
  ...
  truthLayer: () => [],
  physicalRules: (state) => buildSupplementsPhysicalRules(state),
  industryProps: () => ({}),
  compositionRules: () => [],
  resolvePackagingBehavior: (state) => {
  ...
  getAllowedMotions: (productType) => {
    const allowed: Array<'static' | 'opened' | 'dispensed' | 'falling' | 'spilled' | 'pouring'> = [
      'static',
      'opened',
      'dispensed',
    ];
