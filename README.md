...
1. Install dependencies:
   `npm install`
2. Copy `.env.example` to `.env.local` and set `VITE_GEMINI_API_KEY` to your Gemini API key (or start the dev server and paste a key into the in-app prompt). You can also add `VITE_GOOGLE_CLIENT_ID` (from Google Identity Services) to enable one-click Google login, plus comma-separated `VITE_ADMIN_EMAILS` for accounts that should bypass plan limits.
3. Run the app:
   `npm run dev`
