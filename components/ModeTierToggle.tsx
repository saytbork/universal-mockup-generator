...
const ModeTierToggle: React.FC<ModeTierToggleProps> = ({ modeTier, onToggle }) => {
    return (
        <div className="flex items-center justify-center gap-3 py-4 px-2 border-b border-border mb-4">
            <button
                type="button"
                onClick={() => onToggle('basic')}
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${modeTier === 'basic'
                        ? 'bg-accent text-white shadow-accent scale-105 duration-500'
                        : 'bg-surface text-textSecondary hover:bg-surfaceTint hover:text-textPrimary'
                    }`}
            >
                BASIC
            </button>
