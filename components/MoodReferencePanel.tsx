...
const MoodReferencePanel: React.FC<MoodReferencePanelProps> = ({
  ...
  return (
    <div className={`relative bg-gray-50 p-6 rounded-lg shadow-lg border border-gray-200 flex flex-col gap-4 h-full ${disabled ? 'opacity-60' : ''}`}>
      ...
      <label
        className={`border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center justify-center gap-3 ${disabled ? 'cursor-not-allowed pointer-events-none' : 'cursor-pointer hover:border-indigo-600'} transition`}
        onDragOver={(event) => {
          if (disabled) return;
      ...
      {previewUrl && (
        <div className="flex flex-col gap-3">
          ...
          <img src={previewUrl} alt="Mood reference" className="rounded-lg max-h-48 object-cover border border-gray-200" />
        </div>
      )}
      {palette.length > 0 && (
        <div>
      ...
      {summary && (
        <p className="text-sm text-indigo-600 bg-indigo-50 border border-indigo-600/30 rounded-lg p-3">
