  ...
  return (
    <div className={`space-y-3 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-indigo-600">Custom clothes</p>
          <p className="text-[11px] text-gray-600">Upload a reference outfit or tap a preset to keep it raw and real.</p>
        </div>
        {uploadPreview && (
          <button
            type="button"
      ...
      <label
        htmlFor={inputId}
        className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-white px-4 py-6 text-center text-xs text-gray-600 cursor-pointer hover:border-indigo-600 transition"
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-gray-500" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      {uploadPreview && (
        <div className="rounded-2xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">
          <p className="mb-2 text-[11px] uppercase tracking-[0.3em] text-indigo-600">Reference preview</p>
          <img src={uploadPreview} alt="Clothing reference" className="h-32 w-full rounded-xl object-cover" />
        </div>
      )}
      <div>
        <p className="mb-2 text-[11px] uppercase tracking-[0.3em] text-indigo-600">Quick presets</p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1 relative group">
                  ...
                  {preset.tooltip && (
                    <span className="text-xs text-gray-500 cursor-pointer group-hover:text-gray-900">
                      ⓘ
